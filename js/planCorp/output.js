...
function queryCountInvoice() {
    $.ajax({
        type: "post",
        url: baseUrl + "/fipOperaOutput/terminal/invoice/count?corpId="+corpId,
...
function querySumAmountByMonth() {
    $.ajax({
        type: "post",
        url: baseUrl + "/fipOperaOutput/terminal/invoice/sum?corpId="+corpId,
        ...
        success: function (data) {
            ...
            if(data.returnCode == '200'){
                ...
                $('#outputList').html(tbody);
            }else{
                $('#outputList').html("<tr><td colspan='4' style='text-align: center'>暂无数据！</td></tr>");
            }
        }
    });
}
